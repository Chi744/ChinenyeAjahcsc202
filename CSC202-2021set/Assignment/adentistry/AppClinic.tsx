...
const App2 = (): JSX.Element => {
  ...
  //READ
  const fetchData = () => {
    fetch("http://192.168.37.152:3000/clinics", {
      method: "GET",
    })
      .then((res) => {
        return res.json();
  ...
  const handleRemove = (item: { id: number }) => {
    fetch(`http://192.168.37.152:3000/clinics/${item.id}`, {
      method: "DELETE",
    })
      .then((res) => {
        return res.json();
  ...
  const handleCreateModal = () => {
    ...
    if (id == null) {
      ...
      // UPDATE request
      fetch(`http://192.168.37.152:3000/clinics/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  //UPDATE
  const handleUpdate = (item: { id: any; natureOfAilment?: any; medicinePrescribed?: any; procedureUndertaken?: any; }) => {
